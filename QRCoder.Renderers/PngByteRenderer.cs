...
public class PngByteRenderer : Renderer<PngByteRenderer, PngByteRendererSettings, byte[]>
{
    ...
    public PngByteRenderer(QRCodeData data) : base(data)
    {
    }

    public override byte[] Render(PngByteRendererSettings settings)
    {
        // TODO: settings validation
        using (var png = new PngBuilder())
        {
            var size = (QrCodeData.ModuleMatrix.Count - (settings.DrawQuietZones ? 0 : 8)) * settings.PixelsPerModule;
            png.WriteHeader(size, size, 1, PngBuilder.ColorType.Indexed);
            png.WritePalette(settings.DarkColorRgba, settings.LightColorRgba);
            png.WriteScanlines(DrawScanlines(settings.PixelsPerModule, settings.DrawQuietZones));
            png.WriteEnd();
            return png.GetBytes();
        }
    }
